describe "months", ->
	...
	describe "on calling with zero", ->
		beforeEach -> result = months gameObject, 0
		it "returns the number of passed months", -> expect(result).toEqual 0
		check()
	describe "on calling between integers", ->
		beforeEach -> result = months gameObject, 1105048
		it "returns the number of passed months", -> expect(result).toEqual 25
		check()
	describe "on calling close to a lower bound", ->
		beforeEach -> result = months gameObject, 1095850
		it "returns the number of passed months", -> expect(result).toEqual 25
		check()
	describe "on calling close to an upper bound", ->
		beforeEach -> result = months gameObject, 1136150
		it "returns the number of passed months", -> expect(result).toEqual 25
		check()
